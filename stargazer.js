  ...
  // Execution
    ...
    while ( setStargazer["quantity"] >= quantityCounter ){
        CONTAINER.insertAdjacentHTML(
            'afterbegin', 
            '<' + setStargazer["generateItemTag"] + ' class="' + setStargazer["generateItemClass"] + '" id="' + setStargazer["generateItemClass"] + '-' + quantityCounter +'" style="position:' + setStargazer["setPosition"] + '">' + setStargazer["generateItemContent"] + '</' + setStargazer["generateItemTag"] + '>'
        );
        ++quantityCounter;
    };
    ...
    GENERATEDITEM.forEach( function() {
        ...
        let x = Math.floor((Math.random() * (setStargazer["setBorderGap"] / (-1) + 100)) + 1);
        // defining y coordinate
        let y = Math.floor((Math.random() * (setStargazer["setBorderGap"] / (-1) + 100)) + 1);
